...
def create_images_embeddings_and_store_to_txt(list_of_image_paths, initialized_model, txt_storage_dir):
    ...
    for i in range(0, total_images_count, 2):
...
def create_image_embeddings_and_labels_df(embeddings_pickle_file_path):
    ...
    train_generator = train_datagen.flow_from_directory('sketch_training_dataset/paintings_sketches_by_artist',
                                                        class_mode='categorical')
    file_names_list = []
    labels_list = []
    for file_index in range(0, len(train_generator.filenames), 4):
        ...
        file_name_parts = train_generator.filenames[file_index + 2].split('/')
        file_name = file_name_parts[1][:-4] + '.png'
        file_names_list.append(file_name)
        label = file_name_parts[0]
        labels_list.append(label)
...
def show_images(search_image_path):
    ...
    def _get_artists_amd_images_names_from_image_paths_df(image_df):
        ...
        for i in range(len(image_list)):
            ...
            if image_name_parts[-1] == 'resized.png':
                image_name = '_'.join(image_name_parts[:-1])
                artist_name = '_'.join(image_name_parts[:-2])
            else:
                image_name = '_'.join(image_name_parts[:-2])
                artist_name = '_'.join(image_name_parts[:-3])
            image_name += '.jpg'
    ...
    initial_similar_images_df = get_similar_images_df_from_path(search_image_path, custom_model, 12)
    initial_artists, initial_images = _get_artists_amd_images_names_from_image_paths_df(initial_similar_images_df)

    total_similar_images_df = pd.DataFrame()
    for i in range(len(initial_artists)):
        path = os.path.join('images', initial_artists[i], initial_images[i])
        next_similar_images_df = get_similar_images_df_from_path(path, custom_model, 8 - i)
        total_similar_images_df = total_similar_images_df.append(initial_similar_images_df.iloc[[i]]).append(next_similar_images_df)
    ...
    to_plot_artists_names, to_plot_similar_images_names = _get_artists_amd_images_names_from_image_paths_df(total_similar_images_df)
    ...
    if len(to_plot_artists_names) > 29:
        plot_count = 29
    else:
        plot_count = len(to_plot_artists_names)
    for similar_image_index in range(plot_count):
        path = os.path.join('images', to_plot_artists_names[similar_image_index], to_plot_similar_images_names[similar_image_index])
        print(path)
        image = mpimg.imread(path)
        plt.subplot(5, 6, similar_image_index + 2)
        plt.imshow(image)
...
for s in ['sunflower_sketch.jpg', 'stick_filled.jpg', 'dancing.png']:
    # monet = create_single_image_embeddings('sketch_training_dataset/paintings_sketches_by_artist/Claude Monet/Claude_Monet_2_anime_style.png', custom_model)
    # show_images('sketch_training_dataset/paintings_sketches_by_artist/Caravaggio/Caravaggio_2_anime_style.png')
    show_images('test_images/' + s)
    plt.show()
