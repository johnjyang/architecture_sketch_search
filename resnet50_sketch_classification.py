...
def create_images_embeddings_and_store_to_txt(
    list_of_image_paths, initialized_model, txt_storage_dir
):
    ...
    for i in range(total_images_count):
        ...
        single_image_embedding = create_single_image_embeddings(
            list_of_image_paths[i], initialized_model
        )
...
def search_similar_images_by_path(query_image_path):
    ...
    similar_images_df = get_similar_images_df_from_path(
        query_image_path, custom_model, 30
    )
